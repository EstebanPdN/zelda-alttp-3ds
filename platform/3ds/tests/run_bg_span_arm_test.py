...
with tempfile.TemporaryDirectory(prefix='e7-arm-span-') as td:
    tmp = Path(td)
    source = (root / 'platform/3ds/tests/ppu_bg_span_test.c').read_text()
    ...
    print('PASS ARMv6 machine code: 100,000 rows, USUB16/SEL, alignment, strict priority, flip, guards')
